...
from coinem.expectation_step import ParticleGradientExpectationStep, SoulExpectationStep, SteinExpectationStep
...
# Normalize non-zero entries so that they have mean zero and unit standard across dataset
i = images.std(0) != 0
...
# thus, the effective step size is data.n (800) * original LR
...
for jj, seed in enumerate(seeds):
    ...
    for ll, N in enumerate(N_vals):
        ...
        for kk in range(len(pgd_step_size_list)):
            ...
            results_dir = "neurips_experiments/results/bayes_nn_alternative/summary_results" + "/" + "N_" + str(N) + "/" + "T_" + str(K) + "/" + "LR_" + str(kk)
...
fig_dir = "neurips_experiments/figures/bayes_nn_alternative/compare_methods/"
if not os.path.exists(fig_dir):
    os.makedirs(fig_dir)

# id of best learning rate for each method, based on results above
