## adapted from https://github.com/juankuntz/langevin_particle_autoencoders
...
import numpy as np
import copy
from CoinEM.src import cocob_optimizer
...
class LangevinParticleAutoencoder(keras.Model):
    ...
    def __init__(self,
        ...
        # latent_var_dims).
        self._latent_var_batch_init = None  # Initial latent variables used in update. Built
        # when fit is called. Should be of dims (n_particles * batch_size,
        # latent_var_dims).
        ...
        self.optimizer = cocob_optimizer.COCOB()
    ...
    def _build(self, data: Dataset):
        ...
        self._train_latent_variables_init = copy.deepcopy(self._train_latent_variables)
        self.L_lv = tf.Variable(initial_value=tf.zeros_like(self._train_latent_variables), trainable=False, dtype=tf.float32)
        self.grads_lv_sum = tf.Variable(initial_value=tf.zeros_like(self._train_latent_variables), trainable=False, dtype=tf.float32)
        self.abs_grads_lv_sum = tf.Variable(initial_value=tf.zeros_like(self._train_latent_variables), trainable=False, dtype=tf.float32)
        self.reward_lv = tf.Variable(initial_value=tf.zeros_like(self._train_latent_variables), trainable=False, dtype=tf.float32)
    ...
    def fit(self,
        ...
        self._latent_var_batch_init = tf.Variable(initial_value=tf.zeros(
            shape=(self._n_particles * self._train_batch_size,
                   self._latent_dimensions)))

        self._lv_grads_svgd = tf.Variable(tf.zeros_like(self._latent_var_batch))
    ...
    def train_step(self, data: Tensor) -> dict[str, float]:
        ...
        self._latent_var_batch_init.assign(
            self._train_latent_variables_init.gather_nd(lv_idx))
        ...
        # Compute SVGD gradient(s)

        # extract particles corresponding to each latent variable & then compute SVGD
        lv_1_par_idx = list(np.arange(0, self._train_batch_size * self._n_particles, self._train_batch_size))
        for kk in range(self._train_batch_size):
            lv_kk_par_idx = [id + kk for id in lv_1_par_idx]
            lv_kk_grads_svgd= svgd_grad(tf.gather(self._latent_var_batch, lv_kk_par_idx),
                                        tf.gather(lv_grads, lv_kk_par_idx), h=1)
            self._lv_grads_svgd.scatter_nd_update(tf.expand_dims(tf.constant(lv_kk_par_idx, dtype=tf.int32), 1),
                                                  lv_kk_grads_svgd)

        # Update parameters (COIN)
        self.optimizer.apply_gradients(zip(param_grads, self._decoder.trainable_variables))

        # Update latent variables (COIN SVGD)
        abs_grads_lv = abs(self._lv_grads_svgd)
        self.L_lv.scatter_nd_update(lv_idx, tf.maximum(abs_grads_lv, self.L_lv.gather_nd(lv_idx)))
        self.grads_lv_sum.scatter_nd_update(lv_idx, self.grads_lv_sum.gather_nd(lv_idx) + self._lv_grads_svgd)
        self.abs_grads_lv_sum.scatter_nd_update(lv_idx,
                                                self.abs_grads_lv_sum.gather_nd(lv_idx) + abs_grads_lv)
        self.reward_lv.scatter_nd_update(lv_idx, tf.maximum(self.reward_lv.gather_nd(lv_idx) +
                                                            tf.multiply(self._latent_var_batch -
                                                                        self._latent_var_batch_init,
                                                                        self._lv_grads_svgd),
                                                            tf.zeros_like(self._latent_var_batch)))
        self._train_latent_variables.scatter_nd_update(lv_idx,
                                                       self._latent_var_batch_init + self.grads_lv_sum.gather_nd(
                                                           lv_idx) / (self.L_lv.gather_nd(lv_idx) * (
                                                                   self.abs_grads_lv_sum.gather_nd(
                                                                       lv_idx) + self.L_lv.gather_nd(lv_idx))) * (
                                                                   self.L_lv.gather_nd(
                                                                       lv_idx) + self.reward_lv.gather_nd(lv_idx)))

        # Update latent variables (PGD)
        # lv_lr = self._lv_learning_rate  # Get learning rate
        # noise = tf.random.normal(shape=tf.shape(self._latent_var_batch))
        # self._train_latent_variables.scatter_nd_add(lv_idx, lv_lr * lv_grads + tf.sqrt(2 * lv_lr) * noise)

        # Update latent variables (SVGD)
        # lv_lr =self._lv_learning_rate # Get learning rate
        # self._train_latent_variables.scatter_nd_add(lv_idx, lv_lr * self._grads_x_svgd)
...
def svgd_grad(x, grad, h=1):
    assert x.shape[1:] == grad.shape[1:], 'illegal inputs and grads'
    if len(x.shape) > 2:
        x = tf.reshape(x, (x.shape[0], -1))
        grad = tf.reshape(grad, (grad.shape[0], -1))

    kxy, dxkxy = svgd_kernel(x, h=h)

    svgd_grad = (tf.matmul(kxy, grad) + dxkxy) / tf.reduce_sum(kxy, axis=1, keepdims=True)
