...
def network_from_edgelist(edgelist, n_nodes):
    ...
    Y = sp.coo_matrix(
        (data, (edgelist[:, 0], edgelist[:, 1])), shape=(n_nodes, n_nodes)
    ).toarray()
...
def load_got_edgelists():
    ...
    data = pd.concat(
        [
            pd.read_csv(
                file_name, names=["source", "target", "weight", "season"], skiprows=1
            )
    ...
    data = data.groupby(["source", "target", "season"], as_index=False).agg(
        {"weight": "sum"}
    )
