...
from new_experiments.networks.samples_generator import network_from_dynamic_latent_space
...
@lru_cache()
def make_lookup_table(n_samples=10000, low=0.1, high=2.5, n_bins=100, random_state=42):
    ...
    for b, m in enumerate(mu):
        ...
        ratio[b] = np.array([p_out / p_in, m, p_in / n_samples, p_out / n_samples])

    return ratio


def detection_limit_simulation(
    n_nodes=120, n_time_steps=4, trans_proba=0.2, lmbda=0.8, r=0.5, random_state=42
):
    rng = check_random_state(random_state)
    ...
    for t in range(1, n_time_steps):
        ...
        for group_id in range(2):
            ...
            zt[group_mask] = rng.choice(
                np.arange(2), p=wt[group_id, :], size=np.sum(group_mask)
            )
        ...
        for group_id in range(2):
            ...
            Xt[group_mask, :] = sigma * rng.randn(group_count, 2) + (
