...
from new_experiments.network_likelihoods import directed_network_probas
from new_experiments.latent_space import calculate_distances
...
def forecast_probas_map(X, z, wt, lmbda, mu, intercept):
    ...
    for g in np.unique(z):
        X_ahead += ws[:, g].reshape(-1, 1) * (lmbda * mu[g] + (1 - lmbda) * X)
...
def forecast_probas(
    ...
    for _ in range(n_samples):
        ...
        for g in range(n_groups):
            ...
            zt[group_mask] = rng.choice(np.arange(n_groups), p=wt[g, :], size=ng)

        # sample latent positions
        for g in range(n_groups):
            group_mask = zt == g
            ng = np.sum(group_mask)
            Xt[group_mask, :] = sigma[g] * rng.randn(ng, 2) + (
                lmbda * mu[g] + (1 - lmbda) * X[group_mask, :]
            )

        # calculate one-step ahead probabilities
        probas += expit(intercept - calculate_distances(Xt)) / n_samples

    probas[np.diag_indices(n_nodes)] = 0
    return probas


def network_from_dynamic_latent_space(
    X, intercept=1, coef=1, radii=None, metric="euclidean", random_state=None
):
    rng = check_random_state(random_state)
    ...
    if radii is not None:
        probas = directed_network_probas(dij, radii, intercept[0], intercept[1])
...
def simple_splitting_dynamic_network(
    n_nodes=120,
    n_time_steps=9,
    intercept=1.0,
    lmbda=0.8,
    ...
    # group locations
    all_mus = np.array(
        [[-1.5, 0.0], [1.5, 0.0], [-1.5, 0.0], [1.5, 0.0], [0, 3.0], [0, -3.0]]
    )
    ...
    if is_directed:
        ...
        sigma_shape = 13
    sigmas = np.sqrt(
        1.0 / rng.gamma(shape=sigma_shape, scale=sigma_scale, size=n_groups_total)
    )
    ...
    for group_id in range(n_groups):
        ...
        X0[z0 == group_id, :] = (
            sigmas[group_id] * rng.randn(group_size, 2) + mus[group_id]
        )
    X.append(X0)
    z.append(z0)
    ...
    for t in range(1, time_chunks):
        ...
        for group_id in range(n_groups):
            ...
            zt[group_mask] = rng.choice(
                np.arange(n_groups), p=wt[group_id, :], size=np.sum(group_mask)
            )
        ...
        for group_id in range(n_groups):
            ...
            Xt[group_mask, :] = sigmas[group_id] * rng.randn(group_size, 2) + (
    ...
    with np.errstate(divide="ignore"):
        wt_merge = 1.0 / pairwise_distances(old_mus, mus)
    ...
    for group_id in range(n_groups):
        ...
        group_size = np.sum(group_mask)
        zt[group_mask] = rng.choice(
            n_groups + np.arange(n_groups_new), p=wt_merge[group_id, :], size=group_size
        )
    ...
    for t in range(time_chunks + 1, 2 * time_chunks):
        ...
        for group_id in range(n_groups_new):
            ...
            group_size = np.sum(group_mask)
            zt[group_mask] = rng.choice(
                n_groups + np.arange(n_groups_new), p=wt[group_id, :], size=group_size
            )
    ...
    if is_directed:
        norms = 1.0 / np.linalg.norm(X[0], axis=1)
        norms /= np.max(norms)
...
def merging_dynamic_network(
    n_nodes=120, n_time_steps=5, intercept=1.0, lmbda=0.6, random_state=42
):
    rng = check_random_state(random_state)

    # group locations
    mus = np.array([[-5, 0], [5.0, 0], [0, 0]])
    ...
    for group_id in range(2):
        ...
        X0[z0 == group_id, :] = (
            sigmas[group_id] * rng.randn(group_count, 2) + mus[group_id]
        )
    X.append(X0)
    z.append(z0)
    ...
    for t in range(1, n_time_steps):
        if t > 2:
            ...
            wt = np.array(
                [[1 - t / 4.0, 0.0, t / 4.0], [0, 1 - t / 4.0, t / 4.0], [0, 0, 1.0]]
            )
            zt = np.zeros(n_nodes, dtype=np.int)
            for group_id in range(3):
                ...
                group_size = np.sum(group_mask)
                zt[group_mask] = rng.choice(
                    np.arange(3), p=wt[group_id], size=group_size
                )
...
def merging_block_model(
    n_nodes=100, n_time_steps=6, p_in=0.6, trans_proba=0.1, random_state=42
):
    rng = check_random_state(random_state)
    ...
    Z = np.eye(2)[z[0]]
    ZZT = np.dot(Z, Z.T)
    probas = p_in * ZZT + p_in / 5.0 * (1 - ZZT)
    y_vec = rng.binomial(1, probas[indices])
    Y[0][indices] = y_vec
    Y[0] += Y[0].T

    wt = np.array([[1 - trans_proba, trans_proba], [trans_proba, 1 - trans_proba]])
    for t in range(1, n_time_steps):
        # transition nodes
        zt = np.zeros(n_nodes, dtype=np.int)
        for group_id in range(2):
            group_mask = z[t - 1] == group_id
            zt[group_mask] = rng.choice(
                [0, 1], p=wt[group_id, :], size=np.sum(group_mask)
            )
        z.append(zt)
        ...
        if t < 4:
            probas = p_in * ZZT + p_in * ((t + 1) / 5.0) * (1 - ZZT)
        else:
...
def synthetic_static_community_dynamic_network(
    ...
    if simulation_type == "easy":
        ...
        #                [0, 2.0],
        #                [0, -2.0]])
        mus = np.array([[-4, 0], [4, 0], [-2, 0], [2, 0], [0, 5.0], [0, -5.0]])
        sigma_shape = 6
        sigma_scale = 20
        intercept = 1.0
        lmbda = 0.8
    ...
    # group spread
    sigmas = np.sqrt(
        1.0 / rng.gamma(shape=sigma_shape, scale=sigma_scale, size=n_groups)
    )
    # sample initial distribution
    w0 = rng.dirichlet(np.repeat(10, n_groups))  # E[p] = 1 / n_groups
    ...
    for t in range(1, n_time_steps):
        ...
        for group_id in range(n_groups):
            ...
            Xt[group_mask, :] = sigmas[group_id] * rng.randn(group_count, 2) + (
                lmbda * mus[group_id] + (1 - lmbda) * X[t - 1][group_mask, :]
            )
...
def inhomogeneous_simulation(
    n_nodes=120, simulation_type="easy", lmbda=0.9, intercept=1.0, random_state=42
):
    rng = check_random_state(random_state)
    ...
    if simulation_type == "easy" or simulation_type == "custom":
        all_mus = np.array([[-2, 0], [2, 0], [-4, 0], [4, 0], [0, 5.0], [0, -5.0]])
        sigma_shape = 6
        sigma_scale = 0.5
        sticky_const = 20.0
    else:
    ...
    # group spread
    sigmas = np.sqrt(1.0 / rng.gamma(shape=sigma_shape, scale=sigma_scale, size=6))
    ...
    for group_id in range(n_groups_old):
        ...
        group_size = np.sum(group_mask)
        zt[group_mask] = rng.choice(
            np.arange(n_groups), p=wt_merge[group_id, :], size=group_size
        )
    ...
    for t in range(4, 6):
        ...
        for group_id in range(n_groups):
            ...
            group_size = np.sum(group_mask)
            zt[group_mask] = rng.choice(
                np.arange(n_groups), p=wt[group_id, :], size=group_size
            )
    ...
    infinite_mask = ~np.isfinite(wt_merge)
    wt_merge[infinite_mask] = 0
    wt_merge[infinite_mask] = sticky_const * np.max(wt_merge, axis=1)[:n_groups]
    wt_merge /= wt_merge.sum(axis=1).reshape(-1, 1)
...
def synthetic_dynamic_network(
    ...
    for group_id in range(n_groups):
        ...
        zt[group_mask] = rng.choice(
            np.arange(n_groups_new), p=wt_merge[group_id, :], size=group_size
        )
    ...
    for t in range(time_chunks + 1, 2 * time_chunks):
        ...
        for group_id in range(n_groups_new):
            ...
            group_size = np.sum(group_mask)
            zt[group_mask] = rng.choice(
                np.arange(n_groups_new), p=wt[group_id, :], size=group_size
            )
    ...
    infinite_mask = ~np.isfinite(wt_merge)
    wt_merge[infinite_mask] = 0
    wt_merge[infinite_mask] = sticky_const * np.max(wt_merge, axis=1)[indices]
    wt_merge /= wt_merge.sum(axis=1).reshape(-1, 1)
    ...
    for group_id in range(n_groups_new):
        ...
        group_size = np.sum(group_mask)
        zt[group_mask] = rng.choice(
            new_groups, p=wt_merge[group_id, :], size=group_size
        )
    ...
    for t in range(2 * time_chunks + 1, n_time_steps + 1):
        ...
        for idx, group_id in enumerate(new_groups):
            group_mask = z[t - 1] == group_id
            group_size = np.sum(group_mask)
            zt[group_mask] = rng.choice(new_groups, p=wt[idx, :], size=group_size)
